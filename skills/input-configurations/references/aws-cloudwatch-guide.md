...
- [ ] If Federated Identity is in scope: `aws-cloudwatch.yml.hbs` renders a top-level `use_cloud_connectors: {{supports_identity_federation}}` gated on `supports_identity_federation`, next to the flat `role_arn` / access-key fields — no nested `auth.aws:` block; see `references/federated-identity-aws.md` (**Stream template**) -- **HIGH** when in scope
