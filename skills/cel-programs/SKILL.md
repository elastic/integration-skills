...
8. **Secrets** — every sensitive field in `state` must have a corresponding `redact` entry. `state.secret` is always redacted automatically. When `secret_state` is available (v8.19.14 / v9.2.8 / v9.3.3 / v9.4.0), prefer it.
