...
- Set `conditions.kibana.version: "^8.19.0 || ^9.1.0"` for new packages. For Federated Identity / `use_cloud_connectors`, set `conditions.kibana.version: "^9.6.0"` and `conditions.agent.version: "^9.4.0"` instead (elastic/integrations#21007). When adding to an existing package, match the current value instead — only bump if required.
