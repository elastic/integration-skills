...
- **`format_version`, `conditions.kibana.version`, and ECS reference — match existing package versions, do not bump unconditionally:**
  ...
  - **Federated Identity / `provider_permissions`:** this is a justifying feature. Use `format_version: "3.6.4"`, `conditions.kibana.version: "^9.6.0"` and `conditions.agent.version: "^9.4.0"` (see `input-configurations` -> `references/federated-identity-aws.md`; floors per [elastic/integrations#21007](https://github.com/elastic/integrations/pull/21007)). Call the bump out in the changelog.
