...
**Exception — Federated Identity (Cloud Connectors):** if the package will use `provider_permissions` / `var_groups` for AWS identity federation, use `format_version: "3.6.4"`, `conditions.kibana.version: "^9.6.0"` and `conditions.agent.version: "^9.4.0"` (floors per elastic/integrations#21007). Follow `input-configurations` -> `references/federated-identity-aws.md` for the full procedure; see `package-spec` for schema floors.
