...
- [ ] If `var_groups` includes `identity_federation`: `vars` is `[role_arn, supports_identity_federation]` (no `external_id`) and `iac_template_url` points at `cloudformation-federated-identity-aws-<Kibana-floor>.yml` with no `param_ElasticResourceId` — not the GuardDuty-only CFT ([elastic/integrations#20527](https://github.com/elastic/integrations/pull/20527)) -- **HIGH** when in scope (see `input-configurations` -> `references/federated-identity-aws.md`)
