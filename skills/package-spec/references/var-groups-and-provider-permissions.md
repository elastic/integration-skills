  ...
  - name: credential_type
    ...
    options:
      - name: identity_federation
        ...
        provider: aws
        iac_template_url: https://console.aws.amazon.com/cloudformation/home#/stacks/quickcreate?templateURL=https://elastic-cspm-cft.s3.eu-central-1.amazonaws.com/cloudformation-federated-identity-aws-<KIBANA_FLOOR_MINOR>.yml
      - name: direct_access_key
        title: Direct Access Keys
        vars: [access_key_id, secret_access_key]
```

On `identity_federation`, `iac_template_url` is required for the CloudFormation fallback. Replace `<KIBANA_FLOOR_MINOR>` with the package Kibana floor (currently `9.6.0`). There is no `param_ElasticResourceId=RESOURCE_ID` query parameter any more — the external ID was removed from the trust model. The GuardDuty-only `cloudformation-cloud-connectors-guardduty-*.yml` URL is legacy; every shipped package (`aws`, `aws_logs`, `aws_mq`, `aws_securityhub`, `aws_bedrock`) uses the federated-identity template.
...
- `hide_in_var_group_options` on an input hides that input when a given option is selected. Introduced in the first `aws` federation rollout and removed again once those inputs became federation-eligible; no shipped package uses it today.
- Input-level `deployment_modes: ["default"]` pins an input to agent-based mode only — this is how `aws-s3` is handled in `aws_logs` and `aws_bedrock_agentcore`.
...
- `conditions.kibana.version: "^9.6.0"` (or higher) — external-ID-free connector flow
- `conditions.agent.version: "^9.4.0"` (or higher) — `auth.aws` + libbeat cloud connectors shipped in agent 9.4.0, so the agent floor is deliberately lower than the Kibana floor ([elastic/integrations#21007](https://github.com/elastic/integrations/pull/21007))
